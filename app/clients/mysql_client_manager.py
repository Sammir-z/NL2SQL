...
- 一套是保存结构化元数据的 meta 数据库，保存表信息、字段信息、指标信息、字段与指标关系等元数据
- 一套是模拟教学数仓的 dw 数据库，提供真实业务查询要访问的数据，用来验证 SQL、执行 SQL

模块对外提供可复用的客户端管理器和 session 工厂
方便脚本入口 服务层和仓储层按统一方式访问数据库

Engine 更像“连接基础设施”，Session 更像“这次数据库操作的工作上下文”
"""
...
class MySQLClientManager:
    ...
    def init(self):
        ...
        # pool_pre_ping：表示在取出连接时，先做一次可用性检测。
