...
from app.agent.nodes.guard_sql import guard_sql
...
graph_builder.add_node("guard_sql", guard_sql)
graph_builder.add_node("validate_sql", validate_sql)
graph_builder.add_node("correct_sql", correct_sql)
graph_builder.add_node("run_sql", run_sql)
graph_builder.add_node("fail_query", fail_query)
...
def route_after_guard(state: DataAgentState) -> str:
    return "fail_query" if state.get("security_error") else "validate_sql"
...
# 安全校验失败直接终止；数据库校验失败才允许有限次数的模型修正
def route_after_validation(state: DataAgentState) -> str:
    if state.get("security_error"):
        return "fail_query"
    if state.get("error") is None:
        return "run_sql"
    if state.get("validation_attempt", 0) <= state.get("max_sql_retries", 0):
        return "correct_sql"
    return "fail_query"
...
if __name__ == "__main__":
    ...
    async def test():
        ...
        async with (
            ...
            security_context = build_security_context(principal)
            state = DataAgentState(
                query="统计华北地区的销售总额",
                validation_attempt=0,
                max_sql_retries=security_context.max_sql_retries,
                security_error=None,
                error=None,
                validated_sql="",
            )
